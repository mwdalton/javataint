...
import com.siegebrk.sql.MockDatabaseMetaData;
import com.siegebrk.sql.MockConnection;
import com.siegebrk.sql.MockResultSet;
import com.siegebrk.sql.MockRowSet;
import com.siegebrk.sql.MockStatement;
...
public class HypersonicSqlTest
{
    ...
    private final HypersonicSqlValidatorTest hs;

    private final List connections,
                       statements,
                       rowsets;

    public HypersonicSqlTest(int maxlen, SafeRandom sr) {
        this.maxlen = maxlen;
        this.sr = sr;
        ...
        int dbMin = sr.nextInt(20);

        dmd = new MockDatabaseMetaData("HSQL Database Engine", dbMaj, dbMin);
        hs = new HypersonicSqlValidatorTest(maxlen, sr);
    }
    ...
    private String randQuery() {
        ...
        while (query.length() < len) 
            hs.appendRandomToken(query, null);

        return query.toString();
    }

    public boolean pendingException() {
        return hs.pendingException();
    }
    ...
    public static void main(String[] args) {
        ...
        HypersonicSqlTest hs;
        String logfile = "HypersonicSqlTest.log";
        PrintStream ps = null;
        ...
        for (int i = 0; i < args.length; i++) {
           if (args[i].equals("-s"))
               ...
               System.out.println("Usage: java HypersonicSqlTest "
                       + "[-s randomSeed] "
        ...
        hs = new HypersonicSqlTest(maxlen, new SafeRandom(seed));
