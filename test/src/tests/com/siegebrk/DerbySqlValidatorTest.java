...
public class DerbySqlValidatorTest
{

    private final int maxlen;
    private final SafeRandom sr;
    private final boolean blockComments;
    private final DerbySqlValidator v;
    private boolean exception;

    public DerbySqlValidatorTest(int maxlen, SafeRandom sr, 
                                 boolean blockComments)
    {
        this.maxlen = maxlen;
        this.sr = sr;
        this.blockComments = blockComments;
        this.v = new DerbySqlValidator(blockComments);
        clearException();
    }
    ...
    private void appendBlockComment(@StringBuilder@ query, boolean nested) {
        ...
        for (int i = 0; level > 0 && i < len; i++) {
            ...
            /* If block comments are not enabled, this isn't really a comment, 
             * so don't accidentally insert single/double quotes/etc.
             */
            if (!blockComments)
                c = 'a' + sr.nextInt(26);
    ...
    public static void main(String[] args) {
        ...
        SafeRandom sr;
        DerbySqlValidatorTest qt;
        String logfile = "DerbySqlValidatorTest.log";
        PrintStream ps = null;
        ...
        for (int i = 0; i < args.length; i++) {
           if (args[i].equals("-s"))
               ...
               System.out.println("Usage: java DerbySqlValidatorTest "
                       + "[-s randomSeed] "
        ...
        qt = new DerbySqlValidatorTest(maxlen, sr, sr.nextBoolean());
