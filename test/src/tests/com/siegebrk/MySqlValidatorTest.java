...
public class MySqlValidatorTest
{
    ...
    private final MySqlValidator v;
    private boolean exception;

    public MySqlValidatorTest(int maxlen, SafeRandom sr, boolean ansiQuotes,
                              boolean noBackslashEscapes) 
    {
        ...
        this.v = new MySqlValidator(ansiQuotes, noBackslashEscapes);
        clearException();
    }
    ...
    private void appendSingleQuoteLiteral(@StringBuilder@ query)
    {
        ...
        char quoteChar = '\'';
        if (sr.nextBoolean() && !ansiQuotes)
            quoteChar = '"';

        char prev = ' ';
       
        if (query.length() > 0)
        prev = query.charAt(query.length() - 1);
        ...
        for (int i = sb.length(); esc || i < len + 1; i++) {
            ...
            } else if (c == '\\' && !noBackslashEscapes && !esc) {
                esc = true;
            } else if (esc) {
    ...
    private void appendOther(@StringBuilder@ query)
    {
        ...
        for (int i = 0; i < len; i++) {
            ...
            else if (prevc == '-' && Character.isWhitespace(
                                                            //[ifJava4]
                                                            (char)
                                                            //[fiJava4]
                                                            c)) {
        ...
        if (lastc == 'e' || lastc == 'E' || lastc == '/' || lastc == '*' 
                || lastc == '-' || b.get(sb.length()-1) 
                || (lastc >= '0' && lastc <= '9'))
            sb.append(' ');
    ...
    public void test()
    {
        ...
        while(query.length() < len) {
            ...
            if (e != null || pendingException()) {
                ...
                System.out.println("No backslash escapes: "
                                   + noBackslashEscapes);
                System.out.println("ANSI Quotes: " 
                                   + ansiQuotes);
    ...
    public static void main(String[] args) {
        ...
        SafeRandom sr;
        MySqlValidatorTest qt;
        String logfile = "MySqlValidatorTest.log";
        PrintStream ps = null;
        ...
        for (int i = 0; i < args.length; i++) {
           if (args[i].equals("-s"))
               ...
               System.out.println("Usage: java MySqlValidatorTest "
                       + "[-s randomSeed] "
        ...
        qt = new MySqlValidatorTest(maxlen, sr, sr.nextBoolean(), 
                                    sr.nextBoolean());
