...
public class OracleSqlTest
{
    ...
    private final OracleSqlValidatorTest os;

    private final List connections,
                       statements,
                       rowsets;

    public OracleSqlTest(int maxlen, SafeRandom sr) {
        this.maxlen = maxlen;
        this.sr = sr;
        ...
        int dbMaj = 9 + sr.nextInt(3);
        int dbMin = sr.nextInt(6);

        dmd = new MockDatabaseMetaData("Oracle", dbMaj, dbMin);
        os = new OracleSqlValidatorTest(maxlen, sr, dbMaj >= 10, dbMaj >= 10);
    }
    ...
    private String randQuery() {
        ...
        while (query.length() < len) 
            os.appendRandomToken(query, null);

        return query.toString();
    }

    public boolean pendingException() {
        return os.pendingException();
    }
    ...
    public static void main(String[] args) {
        ...
        OracleSqlTest ot;
        String logfile = "OracleSqlTest.log";
        PrintStream ps = null;
        ...
        for (int i = 0; i < args.length; i++) {
           if (args[i].equals("-s"))
               ...
               System.out.println("Usage: java OracleSqlTest "
                       + "[-s randomSeed] "
        ...
        ot = new OracleSqlTest(maxlen, new SafeRandom(seed));
